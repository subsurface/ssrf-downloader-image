...
# only ever run this if this is an subsurface downloader image - not when this is an app
...
names=("${host_name}.local" "${host_name_no_dash}.local" "ssrf-downloader.local")
echo "set up mDNS aliases: ${names[@]}"
service_names=()
for name in "${names[@]}"; do
    service_name="ssrf-avahi-alias@${name}.service"
...
systemctl list-units | grep '^\s*ssrf-avahi-alias@' | awk '{print $1}' | \
